      ...
      <div className="px-4 md:px-6 mb-4">
        <div className="shadow-raised rounded-xl bg-background p-1.5 inline-flex gap-1">
          <button
            ...
            className={`rounded-lg px-4 py-2 text-sm font-outfit font-medium transition-shadow-neu cursor-pointer ${
              activeTab === "requests" ? "shadow-inset text-primary" : "text-muted-foreground hover:shadow-inset"
            }`}
          >
          ...
          <button
            ...
            className={`rounded-lg px-4 py-2 text-sm font-outfit font-medium transition-shadow-neu cursor-pointer ${
              activeTab === "contacts" ? "shadow-inset text-primary" : "text-muted-foreground hover:shadow-inset"
            }`}
          >
                  ...
                  {pending.length > 0 && (
                    <div>
                      ...
                      <div className="space-y-3">
                        {pending.map((r) => <AdminRequestRow key={r.id} request={r} />)}
                      </div>
                    </div>
                  )}
                  {resolved.length > 0 && (
                    <div>
                      ...
                      <div className="space-y-3">
                        {resolved.map((r) => <AdminRequestRow key={r.id} request={r} />)}
                      </div>
                    </div>
                  )}
                </div>
